...
| 5 | 分层依据 | **消息累计 token**（Hot 尾 20k tok / Warm 再 40k tok / Cold 剩余）；不再按消息条数 |
...
## 3-Tier 消息分层（Token-weighted）

每次处理 messages 前，**从尾部往前累计估算 token**，按阈值划分三区。这样一条 10k 字的 `read_file` 结果比一条 "好" 消息占用 100× 的桶位置，更反映真实成本。

```typescript
export interface MessageZones {
  hot: ModelMessage[]    // 尾部累计 ≤ hotTokens — 完整保留
  warm: ModelMessage[]   // 接下去累计 ≤ warmTokens — tool-result payload 可衰减
  cold: ModelMessage[]   // 剩余 — 可进入 summary compact
}

export interface ZoneBoundaries {
  hotTokens: number    // 默认 20000
  warmTokens: number   // 默认 40000（即 warm 覆盖 20k-60k 的尾部切片）
}
...
function estimateMessageTokens(m: ModelMessage): number {
  ...
  return Math.ceil(s.length / 2.5)   // 中文混合 English 经验值
}
...
export function zoneByTokens(
  ...
  let hotTok = 0, warmTok = 0
  for (let i = messages.length - 1; i >= 0; i--) {
    const tok = estimateMessageTokens(messages[i])
    if (hotTok + tok <= boundaries.hotTokens) {
      hot.unshift(messages[i]); hotTok += tok
    } else if (warmTok + tok <= boundaries.warmTokens) {
      warm.unshift(messages[i]); warmTok += tok
    } else {
      cold.unshift(messages[i])
    }
  }
...
**注意**：
- 分层是**计算视图**，不持久化，每轮重算
- **短对话（green tier）**：累计 token 远小于 hotTokens，全部落在 Hot，什么也不做
- **一条极长的 tool-result** 可能独占半个 warm zone——预期行为（它就是成本大头）
- char → token 的 2.5 比是近似。如果 `response.usage` 带了 per-message breakdown（Anthropic cache usage 有），优先用它校准
...
```typescript
// 阈值抬高到"单次读整章小说"以上 — 避免把典型 read_file (一章约 3-5k 字) 不加区别衰减
// 只有跨多章读、大型设定 dump、长审稿结果才会触发
const LARGE_RESULT_TOOLS = {
  read_file:    { minChars: 10000, placeholder: (args, len) => `[read_file('${args.path}'): ${len} chars, re-fetch via read_file() if needed]` },
  read_outline: { minChars: 5000,  placeholder: (args, len) => `[read_outline: ${len} chars snapshot, re-fetch via read_outline()]` },
  read_graph:   { minChars: 8000,  placeholder: (args, len) => `[read_graph: ${len} chars DAG snapshot, re-fetch via read_graph()]` },
  search_lore:  { minChars: 4000,  placeholder: (args, len) => `[search_lore('${args.query}'): ${len} chars of matches, re-query if needed]` },
}
...
export function decayToolResults(messages: ModelMessage[], zones: MessageZones): ModelMessage[] {
  ...
  // 双重门禁：(1) 工具在 LARGE_RESULT_TOOLS 里，(2) 返回 payload > minChars
  // user 消息不衰减（importance-preserved）
}
```
...
1. 设置 `contextManager: 'auto'`、1M 模型、人造 300k tokens 压力（用多次读 20k 字大文档模拟）后：
   - tier 从 green 升到 yellow 后，warm 段里 >10k 字的 `read_file` 结果被占位符替换
   - tokens ratio 不再单调增长（衰减生效）
   - 短对话（total < hotTokens）全部进 Hot zone，什么都不改
